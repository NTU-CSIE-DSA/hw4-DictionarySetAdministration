...
int main(int argc, char *argv[]) {
	...
	if (gentype == "random")
	{
		...
		for(int i=0;i<insert_count;i++) {
			...
			if(dif>=3 && (int)prefix.size()>1) prefix=prefix.substr(0, (int)prefix.size()-1);
			else if(dif<=2 && extra_len>1) prefix=prefix+suffix[0];
		}
		...
		for(int i=insert_count;i<Q;i++) {
			...
			if(type%5==2) {
				...
				cout << "3 " << ask << "\n";
				if ((int)prefix_vec[id].size() > 1) prefix_vec[id] = prefix_vec[id].substr(0, (int)prefix_vec[id].size() - 1);
			}
		}
		// generate a prefix
