...
int main(int argc, char *argv[])
{
	...
	if (gentype == "random")
	{
		...
		for (int i = insert_count; i < Q-1; i++)
		{
	...
	else if (gentype == "tle")
	{
		...
		{
			int type = rnd.next(2, 5);
			if (type == 2) {
				int ans = rnd.next(0, 3);
				string ask = "";
				if (ans != 0)
				{
					ask = inserted[rnd.next(0, insert_sz - 1)];
				...
				int id = rnd.next(0, insert_sz - 1);
				string ask = inserted[id].substr(0, (int)inserted[id].size() - 1);
				cout << "3 " << ask << "\n";
			}
			else if (type == 4)
			{
	...
	else if(gentype=="gentree") {
		string init=argv[7];
		cout << Q << "\n";
		string prefix = "";
		int insert_count = Q * insert_ratio / 10;
		inserted.push_back(init);
		st.insert(init);
		cout<<"1 "<<init<<"\n";
		for (int i = 1; i < insert_count; i++)
		{
			int id=rnd.next(0, (int)inserted.size()-1);
			string prefix=inserted[id];
			char suffix='a'+rnd.next(0, 25);
			prefix.push_back(suffix);
			while((int)prefix.size()>len || st.find(prefix)!=st.end()) {
				id = rnd.next(0, (int)inserted.size() - 1);
				prefix=inserted[id];
				suffix = 'a' + rnd.next(0, 25);
				prefix.push_back(suffix);
			}
			cout << "1 " << prefix << "\n";
		...
		for (int i = insert_count; i < Q - 1; i++)
		{
			...
			{
				string ask = inserted[rnd.next(0, insert_sz - 1)];
				cout << "3 " << ask << "\n";
			}
			else if (type == 4)
			{
